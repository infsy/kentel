...
class ImportDiplomeCommand extends ContainerAwareCommand
{
    ...
    protected function import(InputInterface $input, OutputInterface $output)
    {
        ...
        $data = $this->get($input, $output);
                $output->writeln('<comment>Data loaded-</comment>');
        ...
        foreach($data as $row) {
            ...
            $diplome = $em->getRepository('NurunRhBundle:Diplome')
                ...
                if (empty($diplome))
                {
                    $output->writeln('<error>Le diplome '. $row['description']. ' ne se trouve pas, on le crée</error>');                
                    $diplome = new Diplome();
                ...
                $em->persist($diplome);                
                    ...
                    if (($i % $batchSize) === 0) 
                        ...
                        $now = new \DateTime();
                        $output->writeln(' of diplomes imported ... | ' . $now->format('d-m-Y G:i:s'));
                    }
