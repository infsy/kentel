...
class ImportClientsCommand extends ContainerAwareCommand
{
    ...
    protected function import(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach($data as $row) {
            ...
            if(!is_object($client)){
                $output->writeln('<error>Le client '. $row['Acronyme']. ' doit être créé</error>');   
                $client = new Client();
                $client->setIdentifiant($row['Nom']);
            ...
            if (($i % $batchSize) === 0) 
            {
                ...
                $now = new \DateTime();
                $output->writeln(' of clients imported ... | ' . $now->format('d-m-Y G:i:s'));
            }
