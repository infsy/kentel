...
class ImportPostesCommand extends ContainerAwareCommand
{
    ...
    protected function import(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach($data as $row) {
            ...
            if(!is_object($poste)){
                $output->writeln('<error>Le poste '. $row['poste']. ' doit être créé</error>');   
            ...
            if (($i % $batchSize) === 0) 
            {
                ...
                $now = new \DateTime();
                $output->writeln(' of postes imported ... | ' . $now->format('d-m-Y G:i:s'));
            }
