...
class ImportConseillersRdpCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        ...
        $this
        ->setName('importConseillersRdp:csv')
    ...
    protected function import(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach($data as $row) {
            ...
            $rdp = $em->getRepository('NurunRhBundle:Conseiller')
                       ->findOneByName($row['RDPNom'], $row['RDPPrénom']);
            ...
            if (($i % $batchSize) === 0) 
            {
                ...
                $now = new \DateTime();
                $output->writeln(' of RDP imported ... | ' . $now->format('d-m-Y G:i:s'));
            }
